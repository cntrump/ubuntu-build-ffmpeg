...
RUN git clone --depth=1 -b v1.0.0 https://aomedia.googlesource.com/aom \
    && cd ./aom/build && cmake -DCMAKE_INSTALL_PREFIX=/usr/local -DBUILD_SHARED_LIBS=OFF -DENABLE_TESTS=OFF .. \
...
RUN git clone --depth=1 -b v2.1.0 https://github.com/cisco/openh264.git \
    && cd ./openh264 && make && make install-static && cd .. && rm -rf ./openh264
...
RUN git clone --depth=1 -b v1.0 https://github.com/hoene/libmysofa.git \
    && cd ./libmysofa/build && cmake -DCMAKE_INSTALL_PREFIX=/usr/local -DBUILD_SHARED_LIBS=OFF -DBUILD_TESTS=OFF .. \
...
RUN svn co https://svn.code.sf.net/p/xavs/code/trunk xavs \
    && cd ./xavs && ./configure --prefix=/usr/local --disable-shared --disable-asm \
    && make && make install && cd .. && rm -rf ./xavs

RUN git clone --depth=1 -b release-2.9.3 https://github.com/sekrit-twc/zimg.git \
    && cd ./zimg && ./autogen.sh && STL_LIBS="-lstdc++ -lm" ./configure --prefix=/usr/local --disable-shared --enable-static \
